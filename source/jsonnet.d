 ...
 */
enum LIB_JSONNET_VERSION = "v0.19.1";
...
void jsonnet_max_stack (JsonnetVm* vm, uint v);

/** Set the number of objects required before a garbage collection cycle is allowed. */
void jsonnet_gc_min_objects (JsonnetVm* vm, uint v);
...
void jsonnet_string_output (JsonnetVm* vm, int v);
 ...
 */
alias JsonnetImportCallback = int function (
    void* ctx,
    const(char)* base,
    const(char)* rel,
    char** found_here,
 ...
 */
const(char)* jsonnet_json_extract_string (JsonnetVm* vm, const(JsonnetJsonValue)* v);

/** If the value is a number, return 1 and store the number in out, otherwise return 0.
 */
int jsonnet_json_extract_number (
    JsonnetVm* vm,
    const(JsonnetJsonValue)* v,
    double* out_);

/** Return 0 if the value is false, 1 if it is true, and 2 if it is not a bool.
 */
int jsonnet_json_extract_bool (JsonnetVm* vm, const(JsonnetJsonValue)* v);

/** Return 1 if the value is null, else 0.
 */
int jsonnet_json_extract_null (JsonnetVm* vm, const(JsonnetJsonValue)* v);
 ...
 */
JsonnetJsonValue* jsonnet_json_make_bool (JsonnetVm* vm, int v);
 ...
 */
void jsonnet_json_array_append (
    JsonnetVm* vm,
    JsonnetJsonValue* arr,
    JsonnetJsonValue* v);
 ...
 */
void jsonnet_json_object_append (
    JsonnetVm* vm,
    JsonnetJsonValue* obj,
    const(char)* f,
    JsonnetJsonValue* v);
 ...
 */
alias JsonnetNativeCallback = JsonnetJsonValue* function (
    void* ctx,
    const(JsonnetJsonValue*)* argv,
    int* success);
 ...
 */
char* jsonnet_realloc (JsonnetVm* vm, char* buf, size_t sz);

/** Override the callback used to locate imports.
 */
void jsonnet_import_callback (JsonnetVm* vm, JsonnetImportCallback cb, void* ctx);
...
void jsonnet_max_trace (JsonnetVm* vm, uint v);
 ...
 */
void jsonnet_jpath_add (JsonnetVm* vm, const(char)* v);
 ...
 * \returns Either the error, or a sequence of strings separated by \0, terminated with \0\0.
 */
char* jsonnet_evaluate_snippet_multi (
    JsonnetVm* vm,
    const(char)* filename,
    const(char)* snippet,
    int* error);
 ...
 * \returns Either the error, or a sequence of strings separated by \0, terminated with \0\0.
 */
char* jsonnet_evaluate_snippet_stream (
    JsonnetVm* vm,
    const(char)* filename,
    const(char)* snippet,
    int* error);
 ...
 */
void jsonnet_fmt_indent (JsonnetVm* vm, int n);

/** Indentation level when reformatting (number of spaeces).
 *
 * \param n Number of spaces, must be > 0.
 */
void jsonnet_fmt_max_blank_lines (JsonnetVm* vm, int n);

/** Preferred style for string literals ("" or '').
 *
 * \param c String style as a char ('d', 's', or 'l' (leave)).
 */
void jsonnet_fmt_string (JsonnetVm* vm, int c);

/** Preferred style for line comments (# or //).
 *
 * \param c Comment style as a char ('h', 's', or 'l' (leave)).
 */
void jsonnet_fmt_comment (JsonnetVm* vm, int c);
